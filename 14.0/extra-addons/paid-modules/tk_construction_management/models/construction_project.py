...
class ConstructionSite(models.Model):
    ...
    # Link com Módulo de Projetos
    project_id = fields.Many2one(
        string="Mapeamento de Projeto",
        comodel_name="project.project"
    )
    task_ids = fields.One2many(
        string="Tarefas do Projeto",
    ...
    def create(self, vals):
        ...
        # antes de criar, pesquisar se já não há um projeto criado para essa construção
        project_id = self.env['project.project'].search([
            ('name','ilike',vals.get('name'))
        ])
        if project_id:
            raise UserError("Ops, já existe um projeto criado com o mesmo nome, por favor revise antes de prosseguir.")
        else:
            # Criação do projeto com base no Projeto de Construção
            project_id.create({
                'name': vals.get('name'),
                'allow_subtasks': True,
